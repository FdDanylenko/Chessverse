...
export class Cell {
  ...
  addMove(
    ...
    addLostPiece: boolean
  ) {
    var letter = String.fromCharCode("A".charCodeAt(0) + target.x);
    if (cell.piece) {
      if (
        cell.piece.name.charAt(2).toLowerCase() === "n" &&
        target.x - cell.x === 2
      ) {
        cell.piece?.color === Colors.BLACK
          ? this.board.blackMoves.push("0-0" + additionalInfo)
          : this.board.whiteMoves.push("0-0" + additionalInfo);
      } else if (
        cell.piece.name.charAt(2).toLowerCase() === "n" &&
        target.x - cell.x === -2
      ) {
        cell.piece?.color === Colors.BLACK
          ? this.board.blackMoves.push("0-0-0" + additionalInfo)
          : this.board.whiteMoves.push("0-0-0" + additionalInfo);
      } else {
        if ((targetPiece && targetPiece !== null) || addLostPiece) {
          cell.piece?.color === Colors.BLACK
            ? this.board.blackMoves.push(
                (cell.piece.name.charAt(1) === "n"
                  ? "n"
                  : cell.piece.name.charAt(0) !== "P"
                  ? cell.piece.name.charAt(0).toLowerCase()
                  : cell.x !== target.x
                  ? String.fromCharCode(
                      "A".charCodeAt(0) + cell.x
                    ).toLowerCase()
                  : "") +
                  "x" +
                  letter.toLowerCase() +
  ...
  setAvailableCellsForPasant(target: Cell) {
  ...
  movePiece(target: Cell) {
    this.setAvailableCellsForPasant(target);
    if (this.piece && this.piece?.canMove(target)) {
      ...
      if (thisPiece instanceof Pawn) {
        ...
        pieceBehind = target.board.getCell(
          target.x,
          target.y + oppositeDirection
        ).piece;
      }
      ...
      ((myKing as Cell).piece as King).checkFromWho?.piece?.recheckIfCheck(
        myKing as Cell
      );
      for (let row = 0; row < 8; row++) {
        for (let col = 0; col < 8; col++) {
          ...
          if (
            targetCell.piece &&
            targetCell.piece.canMove(myKing as Cell) &&
            (myKing as Cell).piece?.name === PiecesNames.KING &&
            targetCell.piece.color !== (myKing as Cell).piece?.color
          ) {
      ...
      if (
        ...
        !((myKing as Cell).piece as King).checkFromWho?.piece
      ) {
        ((myKing as Cell).piece as King).isCheck = false;
        if (targetPiece) {
        ...
        let foundedCell: Cell | null = this.board.getCell(
          this.board.previousPasant?.x ? this.board.previousPasant?.x : 0,
          this.board.previousPasant?.y ? this.board.previousPasant?.y : 0
        );
      ...
      let enemyKing: Cell | void = this.board.findKing(
        this.board,
        thisPiece.color === Colors.WHITE ? Colors.BLACK : Colors.WHITE
      );
      this.piece?.recheckIfCheck(enemyKing as Cell);
      if (
        thisPiece instanceof Pawn &&
        target.y === (thisPiece.color === Colors.WHITE ? 0 : 7)
      ) {
        additionalInfo += "Q";
      ...
      if (
        ...
        ((enemyKing as Cell).piece as King).isCheckMate
      ) {
