...
export class King extends Piece {
  ...
  public isCellCheck(target: Cell): boolean {
    ...
    for (let row = 0; row < 8; row++) {
      for (let col = 0; col < 8; col++) {
        ...
        if (
          opponentPiece.piece &&
          opponentPiece.piece.name === PiecesNames.KING &&
          (opponentPiece.piece as King)?.canMoveDowngraded(target) &&
          opponentPiece.piece?.color !== kingPosition.piece?.color
        ) {
          return true;
        }
        if (
          opponentPiece.piece &&
          opponentPiece.piece.name === PiecesNames.PAWN &&
          ((target.x === opponentPiece.x + 1 &&
            target.y === opponentPiece.y + direction) ||
            (target.x === opponentPiece.x - 1 &&
              target.y === opponentPiece.y + direction)) &&
          opponentPiece.piece?.color !== kingPosition.piece?.color
        ) {
        ...
        if (
          ...
          opponentPiece.piece.name !== PiecesNames.KING &&
          opponentPiece.piece?.color !== kingPosition.piece?.color &&
          opponentPiece.piece?.canMoveDowngraded(target)
        ) {
          return true;
        }
      }
  ...
  public canMove(target: Cell): boolean {
    ...
    if (
      ...
      this.cell.board.getCell(7, 7).piece?.name == PiecesNames.ROOK &&
      this.isCheck == false &&
      this.cell.board.getCell(6, 7).isEmpty() &&
      this.cell.board.getCell(5, 7).isEmpty()
    ) {
      return true;
    }
    if (
      ...
      this.canCastling &&
      this.cell.isEmptyHorizontal(target) &&
      this.cell.board.getCell(0, 7).piece?.name == PiecesNames.ROOK &&
      this.isCheck == false &&
      this.cell.board.getCell(1, 7).isEmpty() &&
      this.cell.board.getCell(2, 7).isEmpty() &&
      this.cell.board.getCell(3, 7).isEmpty()
    ) {
      return true;
    }
    if (
      ...
      this.canCastling &&
      this.cell.isEmptyHorizontal(target) &&
      this.cell.board.getCell(7, 0).piece?.name == PiecesNames.ROOK &&
      this.isCheck == false &&
      this.cell.board.getCell(6, 0).isEmpty() &&
      this.cell.board.getCell(5, 0).isEmpty()
    ) {
      return true;
    }
    if (
      ...
      this.canCastling &&
      this.cell.isEmptyHorizontal(target) &&
      this.cell.board.getCell(0, 0).piece?.name == PiecesNames.ROOK &&
      this.isCheck == false &&
      this.cell.board.getCell(1, 0).isEmpty() &&
      this.cell.board.getCell(2, 0).isEmpty() &&
      this.cell.board.getCell(3, 0).isEmpty()
    ) {
      return true;
    }
